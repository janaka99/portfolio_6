          ...
          {specializations.map((specialization, index) => (
            ...
            >
              <Card className="h-full bg-card/40 backdrop-blur-md border-border hover:border-primary/50 shadow-xl hover:shadow-primary/5 transition-all duration-500 overflow-hidden group cursor-none relative">
                ...
                <div className={`absolute inset-0 bg-gradient-to-br ${specialization.color} opacity-0 group-hover:opacity-10 transition-opacity duration-700 pointer-events-none`} />
                
                <CardHeader className="pb-4 relative z-10">
                  <div className="flex justify-between items-start mb-4">
                    <div className={`p-4 rounded-xl bg-muted/50 border border-border/50 group-hover:border-primary/30 group-hover:scale-110 transition-all duration-500 ${specialization.iconColor}`}>
                      {specialization.icon}
                    </div>
                  ...
                  <div className="space-y-4 pt-6 border-t border-border/50">
                    {specialization.highlights.map((highlight, idx) => (
                      <div key={idx} className="flex items-start gap-3">
                        ...
                        <span className="text-sm font-medium text-muted-foreground group-hover:text-foreground transition-colors">{highlight}</span>
                      </div>
                    ))}
                  </div>
                </CardContent>
             ...
             <div className="relative bg-card/80 backdrop-blur-xl rounded-xl p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8">
                <div className="flex items-center gap-6">
                  ...
                  <div className="text-left">
                    <h3 className="text-2xl md:text-3xl font-bold text-foreground mb-2 tracking-tight">Ready to innovate?</h3>
